...
#[derive(Debug, Parser)]
#[command(about = "No-IP Dynamic Update Client", version = clap::crate_version!())]
struct Config {
    ...
    #[arg(long, env = "NOIP_CHECK_INTERVAL", default_value = "5m", value_parser = humantime::parse_duration)]
    check_interval: Duration,

    /// Timeout when making HTTP requests.
    #[arg(long, env = "NOIP_HTTP_TIMEOUT", default_value = "10s", value_parser = humantime::parse_duration)]
    ...
    #[arg(short, long, env = "NOIP_LOG_LEVEL")]
    ...
    /// Import config from noip2 and display it as environment variables.
    #[arg(long, num_args = 0..=1, default_value = "/etc/no-ip2.conf")]
...
fn main() -> anyhow::Result<()> {
    ...
    let log_level = config.log_level.unwrap_or(match config.verbose {
        0 => LogLevel::Info,
        1 => LogLevel::Debug,
        _ => LogLevel::Trace,
    });
